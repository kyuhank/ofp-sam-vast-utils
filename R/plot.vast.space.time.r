...
plot.vast.space.time = function(vast.output, coast.shp, region.shp.list, species = "BET", plot.type = "N", model.start.year = 1952, t.block = 4, plot.obs.grid = FALSE, x.extent = c(95, 295), y.extent = c(-50, 60), scale.trans = "identity", viridis.pal = "D", save.dir, save.name)
{
    ...
    if(plot.type == "obs.cpue")
    {
        ...
        plot.title = "Predicted Encounter Rate"
    ...
    if(plot.obs.grid)
    {
        ...
        if(plot.type %in% c("N","obs.cpue","pred.cpue","bin","pos"))
        {
            ...
            ggplot2::scale_fill_viridis_c(legend.title, option = viridis.pal, trans = scale.trans) +
