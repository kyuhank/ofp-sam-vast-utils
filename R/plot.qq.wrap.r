...
{	
    ...
    {
        stop("Only defined for models using a delta-lognormal or delta-gamma error structure")
    }
    ...
    # Jim Thorson's FishStatsUtils::plot_quantile_diagnostic
    {
        ...
        ## get prediction of individuals caught (rate * area swept)
        ...
        if(error.structure == "dln")
        {
            ...
            mean_y = gamma.scale * (1/pow(Report$SigmaM[1,1], 2))
            y = rgamma(n = nsim*length(Which), shape = rep(1/pow(Report$SigmaM[1,1], 2), each = nsim), scale = gamma.scale)
            dim(y) = c(nsim, length(Which)) ## draws in rows, observations in columns
            var_y = apply(y, 2, var) ## estimate of variance
    ...
    if(missing(save.dir))
    {
        ...
        png(filename = paste0(save.dir, "qq.", error.structure, ".png"), width = 9.5, height = 5.5, res = 300, units = "in", bg = "white", type = "cairo")
        par(mfrow = c(1,2), mar = c(4,5,1,1), las = 1)
        qq.bin = calc.qq.binom(vast.output)
        ab = qqnorm(qq.bin, plot.it = FALSE)
        qqnorm(qq.bin, main = 'Encounter-rate component', col = scales::alpha('dodgerblue4', 0.3), cex.axis = 1.5, cex.lab = 1.5)
